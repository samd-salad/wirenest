...
- **Small MCP surface**: 13 tools across two namespaces (`sot.*` and `wiki.*`) plus live-API helpers for Pi-hole and pfSense. Narrow tools (`sot.get`, `sot.list`, `sot.dependents`, `sot.changes_since`). No `sot.query(sql)`, no separate `log.*` namespace, no curated `v_*` view layer. Agents hit the same Drizzle queries that back the UI.
...
- **Unified sidebar with service co-location**: one tree with services and wiki pages by type. Click pfSense, the tab opens docked beside its wiki page. Per-service session partitioning, process isolation, TOFU cert handling, password autofill with origin check. Co-location is the shell's one job. Not a management console, not a dashboard.
...
The DB is visible to humans through the existing device and build pages. The wiki is paired with it via `@sot:` markers that render live values inline. Agents see both through a small MCP server. No enterprise-grade query layer on top.
...
| Agent interface | MCP server (stdio), namespaces `sot.*` / `wiki.*` |
| Sidecar UI | Electron 41 + electron-vite (service tabs docked beside wiki) |
| Frontend | SvelteKit + Svelte 5 + Tailwind v4 |
| Credentials | `safeStorage` (DPAPI / Keychain / libsecret), encrypted blobs in SQLite |
| Tests | Vitest (454 tests across MCP, Electron, and SvelteKit suites) |

## Status

**Phase 1 Foundation**: DONE. Inventory, build tracker, topology view, wiki, initial MCP server.
...
- MCP surface 28 into 13 tools across `sot.*` and `wiki.*` namespaces. No separate `log.*` namespace (folds into `sot.changes_since`). `reason` enforced on every mutation tool. 8 page-type templates + `wiki.create_page`.
...
- `UNIQUE(secret_ref)` + `ON CONFLICT DO UPDATE` makes upserts atomic at SQL. Credential change-log rows store projected metadata with `hasSecret: boolean` and never the blob.
- Strict route-ID validator (`parseRouteId`) across every `/api/[id]/` route rejects `"7.5"`, `"7e10"`, `"-5"`, `"007"` instead of silently truncating.

**Phase 4.5 Web polish**: DONE (2026-04-18).
- Service views keyed by stable service id, kept attached for their lifetime (no detach on tab close), session cookies auto-promoted to persistent. Close and reopen a tab, stay signed in.
- Refresh button + Ctrl+R / F5 on service tabs.
- Password autofill via `webContents.executeJavaScript` with origin-match check. Plaintext stays in main; only a `{filled, reason}` result returns to the renderer.

**Phase 5 Setup wizard**: IN PROGRESS. Cert-trust and credential-save steps wired to real IPC. Initial-discovery step lands with Phase 6.
...
| Document | What |
|---|---|
| [ARCHITECTURE.md](ARCHITECTURE.md) | Technical design, process model |
